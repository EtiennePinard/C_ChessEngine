...
void _updateCastlePerm(Piece pieceToMove, Square from, ChessPosition* state) {
    if (state->castlingPerm == 0) { return; }

    Square kingSideWhiteRookPosition = H1;
    Square queenSideWhiteRookPosition = A1;
    Square kingSideBlackRookPosition = H8;
    Square queenSideBlackRookPosition = A8;
