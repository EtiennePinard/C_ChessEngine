...
int Bot_staticEvaluation(ChessPosition position) {
    ...
    if (numMoves == 0) {
        if (inCheck) {
            // The player has lost
            return position.colorToGo == BLACK ? BOT_INFINITY : BOT_MINUS_INFINITY;
        }
        // It is a stalemate
        return 0;
    }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        int opposingKingIndex = trailingZeros_64(Board_bitBoardForPiece(position.board, Piece_makePiece(BLACK / (i + 1), KING)));
        kingAttacks = kingMovementMask[opposingKingIndex];
        friendlyBitBoard = Board_specificColorBitBoard(position.board, color);

        for (PieceCharacteristics type = PAWN; type <= KING; type++) {

            bitboard = Board_bitBoardForPiece(position.board, Piece_makePiece(color, type));
            ...
            while (bitboard) {
                ...
                if ((currentFileWithoutPiece & Board_bitBoardForPiece(position.board, Piece_makePiece(color, PAWN))) == 0) {
                ...
                case BISHOP:
                    attack = MagicBitBoard_getBishopPseudoLegalMovesBitBoard(
                        square, Board_allPiecesBitBoard(position.board) & bishopMovementMask[square]
                    ) & (~friendlyBitBoard);
                    score += mobilityBonus[type - 2][numBitSet_64(attack)] * view;
                ...
                case ROOK:
                    attack = MagicBitBoard_getRookPseudoLegalMovesBitBoard(
                        square, Board_allPiecesBitBoard(position.board) & rookMovementMask[square]
                    ) & (~friendlyBitBoard);
                    score += mobilityBonus[type - 2][numBitSet_64(attack)] * view;
                    // Adding more if the piece blocks the king
                    score += reducingKingMovementBonus[type - 3] * numBitSet_64(attack & kingAttacks) * view;
