...
void Engine_playMove(Move move, ChessGame* game) {
  ...
  Piece pieceToMove = Board_pieceAtIndex(game->currentPosition.board, from);
  ...
  // Handling move
  Board_togglePieceAtIndex(&game->currentPosition.board, from, pieceToMove);
  newZobristKey ^= zobristRandomNumber.pieces[(int) pieceToMove - 9][from]; // Removing piece to move
  
  Piece capturePiece = Board_pieceAtIndex(game->currentPosition.board, to);
  if (capturePiece != NOPIECE) { // A piece has been captured!
    Board_togglePieceAtIndex(&game->currentPosition.board, to, capturePiece);
    newZobristKey ^= zobristRandomNumber.pieces[(int) capturePiece - 9][to]; // Removing captured piece
  }
  
  Board_togglePieceAtIndex(&game->currentPosition.board, to, pieceToMove);
  ...
  case EN_PASSANT:
    ...
    piece = Board_pieceAtIndex(game->currentPosition.board, enPassantPawnIndex);
    Board_togglePieceAtIndex(&game->currentPosition.board, enPassantPawnIndex, piece);
  ...
  case KING_SIDE_CASTLING:
    rookIndex = from + 3;
    piece = Board_pieceAtIndex(game->currentPosition.board, rookIndex);
    Board_togglePieceAtIndex(&game->currentPosition.board, rookIndex, piece);
    newZobristKey ^= zobristRandomNumber.pieces[(int) piece - 9][rookIndex]; // Removing rook
    Board_togglePieceAtIndex(&game->currentPosition.board, to - 1, piece);
  ...
  case PROMOTE_TO_QUEEN: 
    Board_togglePieceAtIndex(&game->currentPosition.board, to, Piece_makePiece(colorToGo, QUEEN));
  ...
  case PROMOTE_TO_KNIGHT: 
    Board_togglePieceAtIndex(&game->currentPosition.board, to, Piece_makePiece(colorToGo, KNIGHT));
  ...
  case PROMOTE_TO_ROOK: 
    Board_togglePieceAtIndex(&game->currentPosition.board, to, Piece_makePiece(colorToGo, ROOK));
  ...
  case PROMOTE_TO_BISHOP: 
    Board_togglePieceAtIndex(&game->currentPosition.board, to, Piece_makePiece(colorToGo, BISHOP));
